...
# Un programa BASIC es una serie de sentencias. Se representa como un
# diccionario de tuplas indexadas por número de línea.
...
# Esta regla captura errores catastróficos. Devuelve None.
...
# Formato de todas las sentencias BASIC.


def p_statement(p):
    '''statement : INTEGER command NEWLINE'''
    if isinstance(p[2], str):
        print("%s %s %s" % (p[2], "EN LÍNEA", p[1]))
        p[0] = None
        p.parser.error = 1
    else:
        lineno = int(p[1])
...
# Línea en blanco (solo número de línea)
...
# Manejo de sentencias mal formadas


def p_statement_bad(p):
    '''statement : INTEGER error NEWLINE'''
    print("SENTENCIA MAL FORMADA EN LÍNEA %s" % p[1])
    p[0] = None
    p.parser.error = 1
