# Este archivo provee el soporte de ejecución para un programa BASIC.
# Asume que el programa fue analizado con basparse.py
...
class BasicInterpreter:
    ...
    # Inicializa el intérprete. prog es un diccionario
    # que contiene pares (línea, sentencia)
    def __init__(self, prog):
        self.prog = prog

        self.functions = {           # Tabla de funciones integradas
    ...
    # Recolectar todas las sentencias DATA
    def collect_data(self):
        self.data = []
        for lineno in self.stat:
            if self.prog[lineno][0] == 'DATA':
                self.data = self.data + self.prog[lineno][1]
        self.dc = 0                  # Inicializar el contador de datos

    # Verificar sentencias END
    def check_end(self):
        has_end = 0
        for lineno in self.stat:
            if self.prog[lineno][0] == 'END' and not has_end:
                has_end = lineno
        if not has_end:
            print("NO HAY INSTRUCCIÓN END")
            self.error = 1
            return
        if has_end != lineno:
            print("END NO ES LA ÚLTIMA INSTRUCCIÓN")
            self.error = 1

    # Verificar bucles FOR/NEXT
